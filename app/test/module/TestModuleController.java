...
public class TestModuleController implements MethodInterceptor {
        ...
        if (method.getName().startsWith("_")) {
            ...
            for (Method m : methods) {
                ...
                if (m.getName().equals(controllerMethodName) && m.getModifiers() == 9 && m.getReturnType() == void.class) {
                    ...
                    return executeFunction(method, objects);
                }
            }
            throw new RuntimeException("could not find matching method in controller : [" + superclzz.getCanonicalName() + "#" + controllerMethodName + "]");
        } else {
    ...
    private Object executeFunction(Method method, Object[] objects) throws Exception {
        ...
        if (TestModuleResponse.class == method.getReturnType()) {
            return lastResponse;
        } else if (TestResponse.class.isAssignableFrom(method.getReturnType())) {
            TestResponse response = (TestResponse) method.getReturnType().newInstance();
            response.initialize(lastResponse);
            return response;
        } else {
            return Void.TYPE;
        }
